...
public class SecurityConfig {
    ...
    protected SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable().headers().frameOptions().disable()
                ...
                .requestMatchers("/", "/css/**", "/images/**", "/js**", "/h2-console/**", "/profile").permitAll() // "/"등 지정된 URL들은 permitAll() 옵션으로 전체 열람 권한 부여
                .requestMatchers("/api/v1/**").hasRole(MyRole.USER.name()) // 해당 주소는 USER 권한을 가진 사람만 가능
